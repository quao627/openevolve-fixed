"""Evaluator for first autocorrelation inequality — minimize C1."""
...
c1_computed = float(np.max(autoconv)) / (integral_f ** 2)
if abs(c1_computed - float(c1_achieved)) > 1e-3:
    print(json.dumps({{"error": f"C1 mismatch: {{c1_achieved}} vs {{c1_computed}}"}})); sys.exit(0)
score = {BENCHMARK} / c1_computed
print(json.dumps({{"combined_score": score, "c1": c1_computed, "eval_time": eval_time}}))
"""
    try:
